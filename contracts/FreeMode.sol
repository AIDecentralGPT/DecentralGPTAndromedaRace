...
import "../interfaces/IDbcStaking.sol";

/// @custom:oz-upgrades-from NFTStakingOld
contract FreeMode is  OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IDbcStaking public dbcStaking;
    ReportItem[] public reports;
    ...
    uint public amountDBCForReport;
    ...
    function initialize(
        ...
        amountDBCForReport = 1000e18;

        dbcStaking = IDbcStaking(_dbcStaking);
        if (phaseLevel == 1) {
    ...
    function stake(
        ...
        IMachineInfos.MachineInfo memory _info = machineInfos.getMachineInfoTotal(dockerId,false);
        require(_info.machineOwner == msg.sender,"not owner");
        require(dbcStaking.freeGpuAmount(dockerId) > 0,"no free gpu");
     
        totalGpuCount += 1; //one dockerId one gpu
        if (startAt > 0 ) {
        ...
        require(!stakeInfo.isStaking, "docker already staked");
        ...
        isMachineStaked[dockerId] = true;
                ...
                dbcStaking.reportStakingStatus(dockerId,1,true);
    ...
    function report(string calldata dockerId) external payable {
        require(msg.value == amountDBCForReport,"value error");
        require(!isReport[dockerId],"reporting");
        isReport[dockerId] = true;
    ...
    function verifyReportItem(uint _id,bool _isApprove) external {
        ReportItem storage _item = reports[_id];
        uint _totalVote = _item.approve + _item.reject;
        require(_totalVote < 3,"finish");
        require(isNotary[msg.sender],"not notary");
        require(!isNoraryVote[msg.sender][_id],"voted");
        isNoraryVote[msg.sender][_id] = true;
        ...
        if(_totalVote ==2){
            if(_item.approve > _item.reject){
                //todo finish approve todo
...
//  * @param dockerId 
//  * @param _account 
//  * @return all released,如果质押的数量不足设置的值,这部分总的释放的要先减去缺少质押的,之后才是可领取的
//  * @return available
//  */
...
function _calculateReleaseReward(string memory dockerId)
    ...
    function unStakeAndClaim(
        ...
        dbcStaking.reportStakingStatus(dockerId,1,false);
    ...
    function _unStakeAndClaim(
        ...
        // for (uint256 i = 0; i < nftStakeIds[dockerId][msg.sender].length; i++) {
